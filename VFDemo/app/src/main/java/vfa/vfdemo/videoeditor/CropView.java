...
public class CropView extends ImageView {
    ...
    private CropView.TouchArea mTouchArea = CropView.TouchArea.OUT_OF_BOUNDS;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                onMove(event);
                if (mTouchArea != CropView.TouchArea.OUT_OF_BOUNDS) {
    ...
    private void onUp(MotionEvent e) {
        ...
        mTouchArea = CropView.TouchArea.OUT_OF_BOUNDS;
        invalidate();
    }
    ...
    private void checkTouchArea(float x, float y) {
        if (isInsideCornerLeftTop(x, y)) {
            mTouchArea = CropView.TouchArea.LEFT_TOP;
        ...
        if (isInsideCornerRightTop(x, y)) {
            mTouchArea = CropView.TouchArea.RIGHT_TOP;
            if (mHandleShowMode == ShowMode.SHOW_ON_TOUCH) mShowHandle = true;
            if (mGuideShowMode == ShowMode.SHOW_ON_TOUCH) mShowGuide = true;
            return;
        }
        if (isInsideCornerLeftBottom(x, y)) {
            mTouchArea = CropView.TouchArea.LEFT_BOTTOM;
            if (mHandleShowMode == ShowMode.SHOW_ON_TOUCH) mShowHandle = true;
            if (mGuideShowMode == ShowMode.SHOW_ON_TOUCH) mShowGuide = true;
            return;
        }
        if (isInsideCornerRightBottom(x, y)) {
            mTouchArea = CropView.TouchArea.RIGHT_BOTTOM;
            if (mHandleShowMode == ShowMode.SHOW_ON_TOUCH) mShowHandle = true;
            if (mGuideShowMode == ShowMode.SHOW_ON_TOUCH) mShowGuide = true;
            return;
        }
        if (isInsideFrame(x, y)) {
            if (mGuideShowMode == ShowMode.SHOW_ON_TOUCH) mShowGuide = true;
            mTouchArea = CropView.TouchArea.CENTER;
            return;
        }
        mTouchArea = CropView.TouchArea.OUT_OF_BOUNDS;
    }
